...
- **Thumbnail extraction failure** (non-JPEG embedded thumb, corrupt file): cell shows a static placeholder (a generic file/broken-image icon), not an error banner -- one bad thumbnail shouldn't interrupt browsing the rest of the grid. The existing `#error` banner stays reserved for catalog-level failures (folder import, DB open), consistent with the catalog foundation's error-handling boundary.
